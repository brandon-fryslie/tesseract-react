...
class ControlPanel extends React.Component<ControlPanelProps> {
  ...
  set currentPlayState(value: string) {
  ...
  @action handlePlaylistClick(dom: React.MouseEvent, playlist: PlaylistModel): void {
  ...
  // This will set the current playlist
  // Called with the dom element and the playlist model
  @action handlePlaylistItemClick(dom: React.MouseEvent, playlistItem: PlaylistItemModel): void {
  ...
  @computed get isLooping(): boolean {
  ...
  @action handleEditSceneClick(): void {
  ...
  @action handleResetSceneSettingsClick(): void {
  ...
  @action handleSaveSceneSettingsClick(): void {
    ...
    if (controls.length === 1 && controls[0].fieldName === 'filename') {
      scene.setFilenameValue(scene.clip, controls[0].currentValue as string);
    } else {
      const controlValues = controls.map(control => control.currentValue as number);
      scene.setClipValues(scene.clip, controlValues);
    }
  }

  // Pop up a modal allowing a user to create a new scene
  handleSaveAsSceneSettingsClick(): void {
    UIStore.get().stateTree.sceneModal.isOpen = true;
  }

  // Renders the container for the currently playing Scene
  renderSceneEditButtons(): React.ReactNode {
  ...
  renderPlaylistControlButtons(): React.ReactNode {
    return (
  ...
  renderPlaylistItemList(): React.ReactNode {
...
export default observer(ControlPanel as any);
...
const ControlPanelButton: React.FC<ControlPanelButtonProps> = (props) => {
